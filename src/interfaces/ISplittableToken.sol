...
interface ISplittalbeToken {
    ...
    event ExpirySplit(uint256 indexed delegateTokenId, uint256 previousExpiry, uint256 numberOfSplits);
    ...
    /**
     * @notice Returns the delegate token ID that corresponds to a split token 
     * @return delegateTokenId
     */
    function getDelegateTokenId() external view returns (uint256 delegateTokenId);
